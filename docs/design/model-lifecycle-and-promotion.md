...
> **2026-08-19 delivery boundary:** ADR 0020 將 implementation ticket 的工程驗收與特定模型的營運資格分離。決定性 engineering evidence 可在同一次測試中驗證 gate、核准與五-cycle shadow 狀態機；它不能形成真實 GateDecision／ApprovalDecision／production shadow 的營運主張，也不放寬本文任何 runtime promotion 前置條件。
