...
本專案只供個人及內部團體研究使用。P2 與所有後續 phases／tickets 的必要來源，只能使用官方逐一明示、免帳號、免 API key、免申請文件、免另行書面契約且免付費的公開資料集或官方文件。正式證據保存 dataset／distribution、公開條款或 license 版本、條款內容雜湊、顯名文字、允許用途與有效期間；接受公開條款不被描述成無條件，但不得形成個別採購或外部 entitlement 依賴。

找不到符合此邊界的資料時，產品縮小支援範圍並以 `unavailable`、`degraded` 或 qualification failure 呈現，不能建立「等待採購／法律／合約」gate，也不得改用爬蟲、一般互動網頁、人工下載、測試帳號、商業 feed 或虛構歷史。歷史只來自 platform-observed 或官方公開 archive-attested 證據；訓練 window 與 folds 固定在 `TrainingIntent` 中，但由實際合格歷史決定，統計／類別／校準支援不足時不產生正式模型。
...
The engineering spine cannot produce a formal prediction until Taiwan and United States price history, identity／name history, company actions, calendars, mature labels, model evidence, and serving assignment are qualified under the zero-cost public-source boundary and point-in-time correct. A convenient final-value backfill, provider adjusted close, random train/test split, uncalibrated classifier, or mutable model alias would create plausible output without proving what was known at each cutoff. Researchers need an honest 10＋10 daily price eligibility view, while operators and governors need every market without enough official open evidence to remain explicitly unavailable rather than become an external procurement blocker.

## Solution

Qualify a representative 10＋10 eligibility manifest, activate only market paths with adequate zero-cost official evidence, and expose every other market as explicitly unavailable. For any market with sufficient verified history, build internal adjustment versions and immutable feature snapshots, train class-prior and regularized multinomial logistic TrendForecaster adapters, and govern the first eligible logistic model through BootstrapGatePolicy, internal approval, shadow EOD cycles, atomic service assignment, and formal prediction publication. When verified history is insufficient, the phase bundle records the same complete unavailable evidence without inventing a model or creating a procurement dependency.

## User Stories

1. As a source steward, I want every Taiwan and United States market dataset to carry a versioned official open-data use basis, so that retention, modeling, derivation, display, attribution, and no-application／no-fee status are proven without principal-specific external entitlements.
2. As a source steward, I want an unsupported source to remain unavailable or policy blocked, so that engineering progress never substitutes an unlicensed website, test key, paid feed, or procurement gate.
...
4. As a data engineer, I want a United States EOD path to activate only if an official no-account／no-fee source supplies the required evidence; otherwise the market remains unavailable behind the same contracts.
...
- P2 uses versioned `TWSE-OGDL-OPEN-DATA-01` and corresponding United States public-source bases, not external dependency gates. Each basis records the exact official dataset／distribution, terms, attribution and allowed uses without requiring an account, application, API key, separate contract or fee.
- If a market lacks adequate official open evidence, that market path remains `unavailable`／`policy_blocked`; fixture and technical adapter-contract work may continue, but procurement is not a follow-up and no substitute website, scraper, manual download, test credential or paid feed is allowed.
...
- Taiwan combines allowed OGDL current datasets with a content-addressed platform-observed archive. A United States path activates only when equivalent official zero-cost distributions exist. Both implement the same Collector／Decoder, checkpoint, rate, policy, coverage, and reference-graph interface contracts.
...
- P2 keeps loopback local identity and local secret handling, and may use only locally runnable open-source object, telemetry, alerting and registry adapters. Compose is the required delivery profile; Helm／Kubernetes are optional experiments and cannot block exit.
- The P2 acceptance bundle binds the P1 bundle, public-source basis evidence, qualification manifests, backtest folds, model and gate evidence, approvals, shadow runs, assignment／rollback, EOD SLO, REST／UI results, failure recovery, and Compose results.
...
- Inject late data, missing company action, incomplete coverage, schema drift, checksum corruption, identity ambiguity, public-terms withdrawal／replacement, calibrator insufficiency, approval conflict, assignment race, relay failure, and rollback conditions. Each produces the stable blocked／failure and incident behavior without rewriting prior results.
...
- Paid market data, account／key registration, procurement, external entitlement, or separate source contracts; these are excluded rather than deferred dependencies.
...
2. As a source steward, I want any missing official open category to reduce support explicitly, so that infrastructure readiness cannot override public-source evidence or create a procurement task.
3. As a data owner, I want every listing in the measured support pool checked for identity, calendar, name／symbol history, company actions, sessions, public-source policy, and coverage, so that the pool is not a silent allowlist of easy cases.
...
- P5 is gated by a passing P4 bundle and a versioned support pool whose data is qualified under the product-wide zero-cost boundary. No news, consensus, procurement, account, contract or external-entitlement dependency exists.
- Complete-product data covers the actually supported Taiwan／United States EOD, company actions and identity／name history; official filings／fundamentals; official macro vintages; and any optional official institutional forecast dataflow that is available without registration. Commercial news and company consensus are excluded.
...
- Execute security decision matrices for identity, action grants, public-source policies, data-protection classes, duties, local secret rotation／revocation, source egress, SSRF, rate limits, malicious files, unsafe model artifacts, audit integrity, and policy deletion.
