...
3. As a data engineer, I want the Taiwan official-current adapter and its platform-observed archive reader to share Collector and Decoder contracts, so that source-specific SDK types do not enter DataSupply.
...
- Taiwan combines allowed OGDL current datasets with a content-addressed platform-observed archive. A United States path may use a versioned bundle of qualified zero-cost distributions, while preserving each member's policy、lineage、coverage and gaps. Both implement the same Collector／Decoder, checkpoint, rate, policy, coverage, and reference-graph interface contracts.
...
- Run `P2-TRACE-TW-01` and `P2-TRACE-US-01` through the public source-adapter interfaces, proving checkpoint, rate-limit, policy, coverage, corrections, identity, company-action, and reference-graph behavior; a market without a qualifying zero-cost source must produce the stable unavailable trace.
