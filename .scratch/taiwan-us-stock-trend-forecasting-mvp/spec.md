...
- Keep compose-dev and compose-internal profiles. Both use the same module interfaces, content-addressed artifacts, typed configuration, and domain semantics; only compose-internal carries the capacity and recovery claims actually measured on the local hardware.
...
- Go／No-Go records internal approvals for the platform, data, model, source, and security responsibilities. Model, source-policy, security and any hard-gate veto cannot be overridden by a general ready flag; no external signature service or outside owner is required.
