# product-wide-zero-cost-boundary

Status: locked

Trace ID: `COST-0-01`

本專案只供個人及內部團體研究使用。P2 與所有後續 phases／tickets 的必要來源，可以使用逐資料集合格的公開資料，或官方／非官方提供者的零付費自助方案；自助帳號、API key、secret、token 與標準 click-through 條款可作存取前置，但不得要求付款方式、付費試用、sales 接洽、人工核准、採購或協商契約。正式證據保存 provider、plan、dataset／distribution、條款或 license 版本與雜湊、部署主體分類、顯名、允許用途、保存／刪除限制及有效期間；`$0`、持有憑證或技術上可呼叫都不會自動建立來源使用資格。

找不到符合此邊界的資料時，產品縮小支援範圍並以 `unavailable`、`degraded` 或 qualification failure 呈現，不能建立「等待採購／法律／合約」gate，也不得改用爬蟲、一般互動網頁、人工下載、借用／測試憑證、付費 feed 或虛構歷史。缺少、無效、到期或撤銷的自助來源憑證是程式可管理的 `credential_required`／`policy_blocked` 就緒狀態，不阻止 adapter、REST、UI、工作流程與 fail-closed acceptance 的工程交付；只有真實合格方案與有效憑證可建立 live source 證據。歷史只來自 platform-observed 或合格 archive-attested 證據；訓練 window 與 folds 固定在 `TrainingIntent` 中，但由實際合格歷史決定，統計／類別／校準支援不足時不產生正式模型。

文字模態的完整產品範圍是官方文件，不要求付費新聞或公司級 consensus。必要部署與營運證據必須可由本機／自管開源元件完成；付費 API、雲端服務、外部 OIDC／KMS、商業簽章、獨立滲透測試、Kubernetes、三 failure domains 與跨區復原都不是完成門檻。股票池、容量、SLO 與復原承諾以實際零成本來源及本機硬體可重現的量測為準，不以 600＋1,400 或 2,000 listings 作固定產品聲稱。
...
Trace IDs: `P2-ENTRY-01`, `P2-ENTRY-02`, `P2-TRACE-TW-01`, `P2-TRACE-US-01`, `P2-TRACE-CREDENTIAL-01`, `P2-TRACE-PIT-01`, `P2-TRACE-MODEL-01`, `P2-TRACE-EOD-01`, `P2-EXIT-01`, `GATE-POLICY-01`, `GATE-PIT-01`, `GATE-DATA-01`, `GATE-MODEL-01`, `GATE-SEC-01`, `GATE-OPS-01`, `GATE-DEPLOY-01`, `GATE-UX-01`

## Problem Statement

The engineering spine cannot produce a formal prediction until Taiwan and United States price history, identity／name history, company actions, calendars, mature labels, model evidence, and serving assignment are qualified under the zero-cost source boundary and point-in-time correct. A convenient final-value backfill, provider adjusted close, random train/test split, uncalibrated classifier, or mutable model alias would create plausible output without proving what was known at each cutoff. Researchers need an honest 10＋10 daily price eligibility view, while operators and governors need missing source qualification or credential readiness to remain explicit rather than become an external procurement blocker.

## Solution

Qualify a representative 10＋10 eligibility manifest, let zero-fee providers such as FinMind and Alpaca enter the formal qualification process through one shared credential-managed source boundary, activate only market paths with adequate source-use evidence and credential readiness, and expose every other market as explicitly unavailable. A formal candidate is not a fallback and is not qualified merely because its token or live contract works. For any market with sufficient verified history, build internal adjustment versions and immutable feature snapshots, train class-prior and regularized multinomial logistic TrendForecaster adapters, and govern the first eligible logistic model through BootstrapGatePolicy, internal approval, shadow EOD cycles, atomic service assignment, and formal prediction publication. When source use, credentials or verified history are insufficient, the phase bundle records the same complete blocked／unavailable evidence without inventing a model or creating a procurement dependency.

Ticket engineering acceptance and model operational qualification are separate decisions. A Ticket 09 implementation may finish in one day by using isolated, discarded deterministic contract scenarios plus explicitly labeled deployed engineering evidence through the same public governance seams; this proves policy and lifecycle behavior but does not qualify a real model, create a production assignment, or satisfy the P2 operational exit.

## User Stories

1. As a source steward, I want every Taiwan and United States market dataset to carry a versioned source-use basis, so that retention, modeling, derivation, display, attribution, deployed-principal classification and zero-fee status are proven independently of access credentials.
2. As a source steward, I want an unsupported source to remain unavailable or policy blocked, so that engineering progress never substitutes an unlicensed website, borrowed／test credential, paid feed, or procurement gate.
3. As a data engineer, I want the Taiwan official-current adapter, its platform-observed archive reader, and the authenticated FinMind formal candidate to share Collector and Decoder contracts, so that source-specific SDK types do not enter DataSupply and an unqualified candidate remains fail closed on the same formal path.
4. As a data engineer, I want a United States EOD path to activate when a qualified zero-fee provider or source bundle supplies the required evidence and its credential is ready; otherwise the market remains blocked／unavailable behind the same contracts.
5. As a data engineer, I want a 10＋10 representative stock-pool manifest including ordinary shares, share classes or ADRs, Taiwan name history plus predecessor／successor code transition, applicable market-specific symbol changes, suspensions, company actions, historical delistings, and historical shortened-session regimes, so that qualification is not biased to surviving large issuers or to a nonexistent modern XTAI half day.
...
12. As a model developer, I want quarterly walk-forward folds whose training and validation spans use all eligible verified history recorded in the fold manifest, with a 20-session purge, 20-session embargo, and one-quarter once-only tests, so that model selection cannot see future labels or claim history the free sources did not provide.
...
17. As a model approver, I want the immutable approval policy, artifact, evaluation, gate policy, expected current assignment, approval reason, and independent-review status bound together, so that approval cannot drift to different evidence or imply a review that did not occur.
...
25. As a security administrator, I want local trusted identity, local secret handling, and the same AuthorizationPolicy on workflow and research queries, so that public or authenticated sources cannot be exposed through a weaker path or require an external identity service.
...
27. As a source administrator, I want the application to list credential readiness, accept write-only keys／secrets／tokens, rotate、revoke、validate them and open the provider-owned registration／key-management page, so that credentials can be managed or reapplied without exposing their values or blocking engineering acceptance.

## Implementation Decisions

- P2 uses versioned `TWSE-OGDL-OPEN-DATA-01` and corresponding United States source-use bases, not procurement dependency gates. Each basis records the exact provider、plan、dataset／distribution、terms、deployed-principal classification、attribution、allowed uses、retention／deletion limits and zero-fee status; a self-service account or source credential may be required.
- If a market lacks adequate source-use evidence, credential readiness or historical evidence, that market path remains `credential_required`／`unavailable`／`policy_blocked`; adapter-contract and fail-closed acceptance work may continue, but procurement is not a follow-up and no substitute website, scraper, manual download, borrowed／test credential or paid feed is allowed.
- The 10＋10 stock-pool manifest is versioned. Taiwan selection requires ordinary shares, name history, a predecessor／successor transition between distinct listing identities, suspensions, corporate actions, historical delisting and the documented pre-2001 Saturday shortened-session regime. A scheduled XTAI half day inside a modern seven-year window is explicitly not required because the official calendar evidence shows no such applicable case.
- Taiwan combines allowed OGDL current datasets with a content-addressed platform-observed archive and registers `FINMIND-FREE-TAIWAN-MARKET-DATA-01` as a formal qualification candidate. Its required bundle is exactly `TaiwanStockPrice`、`TaiwanStockTradingDate`、`TaiwanStockDividendResult`、`TaiwanStockDelisting` and `TaiwanStockSplitPrice`; `TaiwanStockInfo` is supplemental current-reference evidence only. A United States path may use a versioned bundle of qualified zero-cost distributions. Every member preserves its own policy、lineage、coverage and gaps, and all market candidates implement the same Collector／Decoder, checkpoint, rate, policy, coverage and reference-graph contracts.
- FinMind uses the same provider catalog、write-only Operations REST／UI、local `SecretProvider`、credential metadata、rotation、revocation、validation and audit seam as Ticket 07. Its bearer token is sent only in the authorization header and never establishes source-use rights, historical completeness or formal qualification. Missing／invalid／expired／revoked credentials prevent provider contact; quota responses are retryable deferred work; incomplete terms、history、schema、calendar、action or lifecycle evidence remains `unavailable`／`policy_blocked`／quarantined.
- FinMind is not an automatic fallback and does not create a second data truth. Yahoo Finance／`yfinance` is not enabled because unofficial client code does not grant the underlying automated data rights. The product does not add `experimental_reference_only` or `experimental_forecast` routes、storage、lineage、model or publication namespaces; a candidate either passes the existing formal qualification gates or remains explicitly blocked.
- A HistoricalAvailabilityClaim is created only by the independent platform qualification workflow delivered in Ticket 08; Ticket 06／07 adapters, Collectors and ordinary DataSupply materialization cannot mint or promote `archive_attested` evidence. `platform_observed` may support daily production and formal history; `archive_attested` may support only explicit historical reconstruction; `published_current_only` remains isolated research. Until that independent workflow verifies an immutable archive manifest, revision semantics, per-member schema／gaps and content bindings, formal historical qualification stays fail closed.
...
- Quarterly folds maximize eligible verified history, record their exact training／validation spans, use a fixed 20-session label purge, fixed 20-session test embargo, and one-quarter once-only tests. Fold count and history depth are evidence, not fixed promises; insufficient statistical, class or calibration support blocks a formal candidate.
...
- Ticket 09 accepts isolated deterministic contract scenarios for BootstrapGatePolicy pass／fail behavior, owner-operated approval behavior and five-cycle shadow state transitions. Positive scenarios may use formally valid values only inside an ephemeral test store; they are scoped by the test harness, discarded after the test and never exported as operational evidence. Deployed delivery evidence must be labeled `engineering_acceptance` and keep `formal_model_qualification=not_claimed` with production serving blocked. Neither form requires real hard-gate measurements, a real approval event or five elapsed trading days.
- Model governance uses an append-only lifecycle ledger and a content-addressed `ModelApprovalPolicyVersion`. `separated_duties` requires an approver distinct from TrainingIntent initiation／execution; the single-principal `owner_operated` profile permits only its designated owner to self-approve and records `independent_review=false`. Both profiles bind exact evidence and expected assignment, preserve every hard-gate veto, require five distinct eligible EOD shadow dates, staged cold load, atomic compare-and-swap service assignment, and a verified rollback target. Registry and MLflow remain rebuildable projections.
...
- The portable single-owner Compose profile is required to complete one scheduled daily operating window, not to keep the host continuously online across the five shadow dates or the seven-day approval validity period. Planned offline maintenance between daily windows is allowed. On restart, collection resumes from the committed checkpoint; public content released while offline receives its actual post-restart `first_observed_at` and cannot enter an earlier cutoff.
- P2 keeps loopback local identity and uses a local open-source `SecretProvider`. Source credential values are write-only and never enter repository、database、typed configuration、work commands、artifacts、REST responses、outbox、logs or traces; only secret references、readiness、version、timestamps and audit evidence are queryable.
- Operations UI／REST supports list readiness、set、rotate、revoke and validate for source credentials. Reapplication opens a versioned provider-owned registration／key-management URL and then accepts a replacement secret; automatic account／key issuance is out of scope unless the provider exposes a separately qualified official API.
- The P2 acceptance bundle binds the P1 bundle, source-use basis and credential-readiness evidence, qualification manifests, backtest folds, model and gate evidence, approvals, shadow runs, assignment／rollback, EOD SLO, REST／UI results, failure recovery, and Compose results.
...
- Run `P2-TRACE-TW-01` and `P2-TRACE-US-01` through the public source-adapter interfaces, proving checkpoint, rate-limit, policy, coverage, corrections, identity, company-action, and reference-graph behavior. The Taiwan trace covers both TWSE OGDL observed history and the FinMind authenticated formal candidate; token readiness and the ten-listing／five-dataset live probe remain distinct from qualification. A market without a qualifying zero-cost source must produce the stable unavailable trace.
- Run `P2-TRACE-CREDENTIAL-01` through Operations UI／REST and the source workflow: no secret value is ever returned, logged or persisted outside `SecretProvider`; missing、invalid、expired、rotated and revoked credentials yield stable readiness and block network use, while replacement credentials do not rewrite prior decisions.
...
- Verify XTAI and United States session mapping, each market's actually applicable shortened sessions, temporary closures, name／symbol history, cross-listing transitions, delistings, exact t+h endpoints, corporate-action adjustment, label maturity, and the prohibition on provider adjusted close as truth.
...
- For Ticket 09 engineering acceptance, verify through `ModelLifecycle.execute` in an isolated ephemeral store that BootstrapGatePolicy passes only when deterministic evidence exceeds class prior by the required amount and every absolute gate passes; verify owner-operated and separated-duties outcomes, exact bindings, expiry and invalidation; any failure preserves blocked serving. These events are test-scoped and discarded, not labeled or exported as a real candidate.
- For Ticket 09 engineering acceptance, execute five distinct and increasing deterministic market-eligible EOD dates through the public shadow seam in an isolated ephemeral store during one test run, including staged load, schema, latency, probability, lineage, replay and corruption behavior. Separately, P2 operational exit still requires five actually observed eligible EOD shadow runs with planned-stop／restart continuation; engineering dates cannot enter production history or satisfy that runtime qualification.
- Inject late data, missing company action, incomplete coverage, schema drift, checksum corruption, identity ambiguity, source-terms withdrawal／replacement, credential failure, calibrator insufficiency, approval conflict, assignment race, relay failure, and rollback conditions. Each produces the stable blocked／failure and incident behavior without rewriting prior results.
...
- Run Compose small smoke from a clean environment and preserve a reproducible P2 acceptance bundle; optional local Kubernetes smoke is informational only.
...
- Paid market data、payment methods、paid trials、sales／manual approval、procurement、negotiated source contracts and automated provider account／key issuance. Self-service zero-fee registration and application-managed credential entry are in scope.
- Yahoo Finance／`yfinance` automated collection and separate `experimental_reference_only`／`experimental_forecast` data、model or publication paths.
...
- Ticket 09 may be engineering-complete while P2 remains formally blocked. Its checked AC 5–7 mean the governance capability is implemented and reproducible, not that a real model has acquired operational qualification.
- `ready-for-agent` means the implementation work is fully specified. Market traces exit only for evidence actually qualified under the product-wide zero-cost boundary; missing markets remain unavailable without an external procurement gate.
...
Price-only forecasts do not let researchers inspect how filings, official announcements, fundamentals, and macro vintages relate to a prediction. Naively adding text would risk treating URLs as document identity, expanding public terms to unrelated pages, leaking later corrections or retrospective annotations into historical folds, mislinking ticker mentions, executing hostile files, or presenting general tone as causal market impact. The product needs a zero-cost multimodal research pilot whose official evidence is bilingual, point-in-time correct, and policy-aware without depending on commercial news.

## Solution

Add the deep DocumentIntelligence implementation and qualified Taiwan／United States official-document, financial-fact, and macro paths. Preserve immutable document evidence separately from versioned derived intelligence, process untrusted content in a constrained sandbox, require confirmed target links for listing features, and publish versioned annotations, events, embeddings, and market-impact assessments. Build multimodal logistic candidates with ablations against the P2 price baseline and expand the research experience to the full Traditional Chinese MVP. `official-documents-only` is the complete intended product scope, not a temporary blocker awaiting paid news.
...
5. As a data engineer, I want Taiwan CBC／DGBAS, BLS v1 and any independently qualified zero-fee institutional distribution to preserve release and vintage, so that later revisions do not rewrite old feature snapshots.
6. As a source steward, I want BEA API、FRED／ALFRED、IMF and similar interfaces judged by their dataset-specific zero-fee terms and credential readiness rather than excluded solely for requiring a key, so that a missing credential remains an explicit optional state rather than an external gate.
...
12. As a researcher, I want exact and near-duplicate relations without deleting source documents, so that repeated reporting does not multiply features or erase publication, attribution, and source-policy differences.
...
21. As a researcher, I want a matrix for every listing in the measured qualified support pool, with search, filtering, sorting, support states, and all three horizons, so that I can find cross-market disagreement and data gaps without a fixed scale promise.
...
26. As a delivery owner, I want the P3 bundle to prove the official-document scope and list excluded commercial modalities, so that absence of paid news is not misreported as a product blocker.

## Implementation Decisions

- P3 requires a passing P2 bundle plus qualified, zero-cost Taiwan and United States official documents, fundamentals, and macro sources. All research surfaces and the acceptance bundle display `official-documents-only` as the final scope.
...
- Taiwan paths cover MOPS／OGDL material information, monthly revenue, and financial summaries. United States paths cover SEC forms and company facts. Macro paths cover CBC＋DGBAS, BLS v1 and optional independently qualified zero-fee institutional distributions. BEA API、FRED／ALFRED or IMF are not excluded solely for requiring self-service credentials, but no such optional source may activate without dataset-specific use qualification and credential readiness.
- Paid commercial news adapters are excluded. A future zero-fee news dataset may enter only through a new versioned source policy and normal qualification; a self-service credential is permitted, but its absence never blocks this product.
...
- The highest seam is an official open document entering DataSupply and DocumentIntelligence, contributing to an immutable feature snapshot and shadow forecast, and becoming visible through policy-filtered research REST／UI plus canonical health and audit evidence.
...
- Verify the complete measured support-pool research experience, policy-filtered evidence, main influences as non-causal attribution, URL-stable state, keyboard navigation, text status, narrow layout, REST snapshot consistency, T+120 batch SLO, and REST latency SLO.
- Exercise correction, withdrawal, policy deletion, derived-impact deletion, backup restore then re-delete, excluded-news state, review queue, evidence-projection lag, and source outage without rewriting prior predictions.
...
- Commercial news ingestion or any claim that official-document scope includes excluded news.
...
- Scale beyond the measured zero-cost source／hardware support pool and final internal cutover.

## Further Notes

- P3 exits in an `official-documents-only` state; excluded commercial news creates no P5 blocker.
...
Implement NeuralTrendForecaster behind the existing deep TrendForecaster seam, first matching the price-only contract and then adding fundamentals, macro, frozen text representations, and quality-aware gated fusion in controlled steps. Govern every candidate through immutable TrainingIntent, three seeds, limited HPO, every statistically eligible walk-forward fold available in the verified-history manifest, six calibrators where sample support permits, baselines and ablations, deterministic attribution, offline reproduction, hard-gate vetoes, internal approval, shadow cycles, atomic promotion, and verified rollback. Insufficient fold or calibration support keeps the candidate unavailable. Add drift assessment and recurring training triggers without allowing the model to update or promote itself.
...
6. As a model developer, I want frozen, locally runnable, open-license, versioned multilingual segment embeddings from DocumentIntelligence, so that TrendForecaster never reads raw text, calls a paid API, or fine-tunes a large encoder.
...
16. As a model approver, I want human approval governed by an immutable separated-duties or designated-owner policy and bound to exact artifact and gate evidence, so that automated completion cannot alter service and absent independent review remains visible.
...
- P4 starts only from a passing P3 bundle, the exact measured support-pool feature schemas and processing bundles, an approved baseline where eligible, a qualified local reproduction runtime, and complete internal staging.
...
- Large multilingual text encoders are frozen, versioned, locally executable, and covered by an approved open-source model license. TrendForecaster receives embeddings and evidence references, never raw text, remote code, paid API calls, or an opportunity to fine-tune the encoder.
...
- Human approval follows the immutable `ModelApprovalPolicyVersion`, binds the exact artifact, evaluation, gate policy, expected assignment and review mode, and expires after seven days or any material evidence change. `separated_duties` keeps the approver distinct from TrainingIntent initiation／execution; `owner_operated` is restricted to its designated owner and records that no independent review occurred.
...
- Test separated-duties rejection, designated-owner self-approval and non-owner rejection, explicit independent-review disclosure, seven-day expiry, evidence invalidation, five distinct shadow dates across planned stops, staged cold load, compare-and-swap conflicts, pinned EOD assignment, rollback-target daily verification, and the prohibition on break-glass promotion.
...
- Expanding formal support beyond the exact qualified support-pool manifest; additional listings may be isolated OOD or shadow evidence only until separately qualified.
- Lowering or renegotiating already approved GatePolicy, label, fold, calibration, safety, SLO, or source-policy semantics.
- Product scale beyond measured zero-cost source／hardware capacity and any optional Kubernetes／cross-region experiment.
...
# phase-5-zero-cost-internal-baseline-deployment
...
A small-profile result does not prove that the research decision support system can safely serve every listing actually supported by zero-cost official sources on the available local hardware. Configuration and dashboards are not evidence. The internal team needs a content-addressed, capacity-qualified deployment artifact and an approved release bundle that proves the data, model, security, operational, backup／restore, and research experience under a measured representative load while preserving fail-closed behavior for every supported listing and source.

## Solution

Qualify the complete stock pool actually covered by zero-cost official sources, retrain, calibrate, backtest, and govern the best eligible model for that measured support profile, then deploy the same content-addressed application through the local Compose profile. Prove local capacity, process／storage fault behavior, security, policy deletion, backup／restore, restart recovery, and staged cutover through immutable reports and fault injection that fits the available hardware. Release only after the internal hard-gate checks pass; otherwise remain unavailable or degraded with exact reasons.

## User Stories

1. As a source steward, I want zero-cost official EOD, company action, identity／name history, filings／fundamentals, macro vintages, and optional official institutional forecasts qualified, so that the product claim matches evidence for every promised category without paid news or consensus.
2. As a source steward, I want any missing qualified zero-fee category to reduce support explicitly, so that infrastructure or credential readiness cannot override source-use evidence or create a procurement task.
3. As a data owner, I want every listing in the measured support pool checked for identity, calendar, name／symbol history, company actions, sessions, source-use policy, credential readiness, and coverage, so that the pool is not a silent allowlist of easy cases.
...
6. As a model governor, I want the model retrained, calibrated, backtested, and approved for the exact measured stock-pool manifest, so that an artifact cannot silently expand its support profile.
...
8. As a researcher, I want the full Traditional Chinese matrix and listing research experience to remain correct at measured supported-pool scale, so that scale does not reduce lineage, support, policy display, or accessibility.
9. As a platform engineer, I want compose-dev and compose-internal to use the same content-addressed deployment artifact and typed configuration, so that deployment profile does not change domain results.
10. As a platform engineer, I want local process, container, database, and object-repository failures exercised, so that restart and fail-closed behavior are measured without requiring Kubernetes or multiple failure domains.
11. As a platform engineer, I want application, orchestration, data, edge, and observability responsibilities separated by local identities, network policy, and resource budgets, so that Compose runtime boundaries support operations without becoming module seams.
...
16. As an operations engineer, I want actual API, relay, Dagster, worker, container, PostgreSQL connection／restart, and object failures injected, so that local resilience is proven by service results rather than unavailable infrastructure.
17. As a security administrator, I want loopback／internal authentication, scoped action grants, local secret rotation, source-restricted egress, and least-privilege storage roles, so that formal access is controlled without an external identity or key-management service.
18. As an auditor, I want content-addressed images and models, SBOM, provenance, optional local signatures, audit-chain verification, authorization decisions, and policy deletion evidence, so that formal internal artifacts and actions can be independently checked with zero-cost tools.
19. As a source steward, I want a policy deletion to stop use, remove content and derivatives, replay after restore, and produce a deletion certificate, so that ordinary retention does not lock content past its latest deletion deadline.
...
21. As a recovery owner, I want local database and object backup loss bounds plus restore time measured on the available hardware, so that internal recovery expectations are empirical rather than cloud promises.
22. As a security administrator, I want only one active deployment generation during restart, restore, cutover, and rollback, so that two local generations cannot both publish formal state.
...
25. As a platform owner, I want deployment rollback to restore the last content-addressed compatible artifact without rolling back canonical data or audit, so that application recovery never rewrites authoritative evidence.
26. As a security assessor, I want reproducible open-source static, dependency, container, dynamic, and manual abuse-case checks with all Critical and High findings remediated and retested, so that release does not require a paid independent assessor.
27. As a governance owner, I want the internal platform／data／model／source／security responsibilities and veto results recorded in the final bundle, so that one person cannot silently override a hard gate without requiring external signatures.
28. As an auditor, I want the final acceptance bundle retained while its referenced formal artifacts remain supported, so that product scope, capacity, source policy, model assignment, security, recovery, and release decisions remain reproducible without a fixed commercial retention term.

## Implementation Decisions

- P5 is gated by a passing P4 bundle and a versioned support pool whose data is qualified under the product-wide zero-cost boundary. No paid news、consensus、payment、sales／manual approval、procurement or negotiated-contract dependency exists; any self-service source credential has an explicit application-managed readiness state.
- Complete-product data covers the actually supported Taiwan／United States EOD, company actions and identity／name history; official filings／fundamentals; official macro vintages; and any optional qualified zero-fee institutional forecast dataflow. Paid commercial news and company consensus are excluded.
...
- Retrain, calibrate, reproduce, backtest, and govern the eligible model against the exact support-pool manifest. The current model is the best artifact that passes all gates; neural is not privileged over logistic.
- Keep compose-dev and compose-internal profiles. Both use the same module interfaces, content-addressed artifacts, typed configuration, and domain semantics; only compose-internal carries the capacity and recovery claims actually measured on the local hardware. The required local availability claim is one scheduled daily operating window with planned offline maintenance between windows, never uninterrupted multi-day uptime or HA.
- Kubernetes, replicated storage, multiple failure domains and second-region recovery are optional zero-cost experiments. They cannot appear in the required capacity, availability or release claim.
- Separate edge, application, orchestration, data, and observability operational responsibilities with the strongest local network restrictions the runtime supports, independent workload identities, database roles, object prefixes, resource budgets, and approved egress. Compose service layout does not redefine application modules.
- The required portable reference uses the existing PostgreSQL and filesystem ObjectRepository under Compose. Optional free PostgreSQL／object replication experiments may be evaluated separately but do not alter the product contract or release gate.
...
- Reserve a measured daily-critical CPU／memory／connection budget during a protection window starting 30 minutes before the first market cutoff. Backfill and training checkpoint, pause, or recreate and cannot consume the reserved local baseline.
- The baseline representative load is the complete qualified support-pool manifest, observed point-in-time history, measured official-document volume, and a documented internal-user／REST workload that the available local hardware can reproduce. An optional stretch profile tests safe degradation without becoming a product promise.
...
- Deployment artifacts are content addressed and include application／UI images, Compose, typed configuration, migrations, SBOM, provenance, dashboards, alert rules, and runbooks. Optional local signatures or Helm files are supplementary. Each work attempt and EOD batch pins one artifact digest; no formal batch mixes versions.
- Staged local releases use role-specific probes, canary research checks, expand／contract migrations, protected-window restrictions, and forward fixes for incompatible schema. Rollback never rolls back the canonical ledger or prediction history.
- Security requires internal trusted identity, scoped grants, local secret handling, least privilege, default-deny where the local runtime supports it, SSRF defenses, content-addressed artifacts, safe model formats, append-only audit, policy deletion, and reproducible open-source／manual security assessment.
- Local resilience acceptance injects application process, container, PostgreSQL connection／restart, object, relay, and orchestration failures and verifies leases, fencing, repair, projection recovery, backup restore, and preserved daily-critical correctness.
...
- Recovery targets are the database／object loss bounds and restore time measured by local backup-and-restore drills. Restart, restore and return-to-service are controlled internal actions with policy revalidation and a monotonically increasing deployment generation; regional failover is not required.
...
- Go／No-Go records internal decisions for the platform, data, model, source, and security responsibilities. In the approved single-owner profile one natural person may carry these responsibilities, but each decision, veto result and lack of independent review remains separately attributable; model, source-policy, security and any hard-gate veto cannot be overridden by a general ready flag. No external signature service or outside owner is required.
- The final bundle binds source-use bases and non-secret credential-readiness metadata, all supported listing qualifications, data／model／prediction lineage, current and rollback models, shadow evidence, measured capacity, restart behavior, security, deletion, local restore, SLO／incidents, UI／REST accessibility, runbook exercises, findings, approvals, and expiry.

## Testing Decisions

- The highest seam is the content-addressed Compose deployment artifact running the measured local baseline workload, observed through EOD publication, REST／UI, canonical ledgers, security decisions, incidents, local recovery, and cutover. Infrastructure component tests supplement but do not replace this seam.
- Qualify every listing in the support-pool manifest and every promised official source category. Verify precise unavailable／degraded behavior when a public dataset is absent, changed, withdrawn or insufficient.
- Run three consecutive baseline workloads covering normal days, applicable half days, daylight-saving transitions, temporary closures, document spikes, schedule overlap, API traffic, backup／restore activity, and background backfill. Use the worst run for hard deadlines and correctness.
...
- Inject API, relay, Dagster, worker, container, PostgreSQL connection／restart, and object faults. Verify lease and fencing, restart recovery, repair, idempotency, and reserved daily correctness by external results.
- Execute security decision matrices for identity, action grants, source-use policies, data-protection classes, duties, source-credential and local-secret rotation／revocation, source egress, SSRF, rate limits, malicious files, unsafe model artifacts, audit integrity, and policy deletion.
- Verify image, model, deployment, SBOM, provenance, optional local signature, vulnerability, license, runtime identity, database role, object path, and network-policy checks with free tools. Critical or High findings remain hard vetoes until remediated and rerun by the internal review workflow.
- Perform periodic local restore evidence and a complete prediction-record lineage restore. Measure database／object loss bounds, restore time, deletion-ledger replay, missing-object behavior, audit-chain restoration, and single deployment generation.
...
- Execute cutover in the approved order and verify a rollback point at every stage. No prior local deployment generation may publish the same market batch while the current generation is active.
- Verify full Traditional Chinese research behavior for the complete measured support pool: search／matrix, listing details, evidence policy, lineage, accessibility, snapshot cursors, ETags, history, and backtests under representative traffic.
- Generate and checksum the final acceptance bundle only after every hard gate and required internal approval succeeds; a failed or blocked attempt produces new immutable evidence and never edits a prior bundle.
...
- Paid cloud IaC, managed-service dependencies, external identity／key management, mandatory Helm／Kubernetes, and regional disaster infrastructure.
...
- `ready-for-agent` means the zero-cost internal baseline work is fully specified; there are no external procurement, cloud-provider or independent-assessment inputs to await.
- Compose is the required complete semantic and measured-capacity profile. Optional Kubernetes experiments do not carry HA／DR commitments.
