...
- Ticket 09 accepts deterministic `engineering_acceptance` evidence for BootstrapGatePolicy pass／fail behavior, owner-operated approval behavior and five-cycle shadow state transitions. This delivery evidence may use fixed historical dates in one test run and does not require real hard-gate measurements, a real approval event or five elapsed trading days; the acceptance bundle must keep `formal_model_qualification=not_claimed` and production serving blocked.
...
- For Ticket 09 engineering acceptance, verify through `ModelLifecycle.execute` that BootstrapGatePolicy passes only when deterministic evidence exceeds class prior by the required amount and every absolute gate passes; verify owner-operated and separated-duties outcomes, exact bindings, expiry and invalidation; any failure preserves blocked serving. The evidence is labeled engineering-only and makes no claim about a real candidate.
- For Ticket 09 engineering acceptance, execute five distinct and increasing deterministic market-eligible EOD dates through the public shadow seam in one test run, including staged load, schema, latency, probability, lineage, replay and corruption behavior. Separately, P2 operational exit still requires five actually observed eligible EOD shadow runs with planned-stop／restart continuation; engineering dates cannot enter production history or satisfy that runtime qualification.
