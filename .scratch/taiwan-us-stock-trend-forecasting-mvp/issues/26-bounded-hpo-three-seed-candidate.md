...
- [ ] 所有 trials 使用相同 chronological training／validation 邊界、預先登錄 seeds 與資源限制；最新八季 test labels、metrics、class balance 及 economics 完全不可見。
...
- [ ] 正式候選重新擬合六 calibrators、執行八季 tests、baselines／ablations、cost scenarios、support slices 與 operational checks，產生 immutable ModelArtifact／EvaluationReport。
