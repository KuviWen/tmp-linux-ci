...
- [ ] 版本化 measured support-pool manifest 依實際合格零成本 coverage 建立，並按產業、規模、流動性、掛牌年齡、股別／ADR、報告制度、文件密度及 full／degraded／unavailable／policy-blocked 支援分層；不承諾固定 100＋100。
...
- [ ] 完整 measured support pool 的正式／shadow EOD 對每掛牌三期間產生機率或明確不可用原因，optional modality 降級不改寫信心定義，必要價量不足時不產生機率。
