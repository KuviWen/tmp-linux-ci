...
- 驗證：`python -m pytest -q`（33 passed）、`python -m mypy src tests`、`python -m ruff check .`、`python -m ruff format --check .`、fresh Alembic upgrade 與 `python -m stock_forecasting.cli acceptance ticket-01 ... --observed-at ...`（`status=passed`，含 adversarial scenarios 與 scenario-correct work／health／audit）。
