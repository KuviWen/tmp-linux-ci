...
- 驗證：完整非 PostgreSQL suite `432 passed, 1 deselected`，PostgreSQL opt-in `1 passed, 432 deselected`；`mypy src tests`（101 source files）、`ruff check .`、`ruff format --check .`、OpenAPI contract、wheel build、Compose config 與 Compose acceptance 均通過。Wheel SHA-256 為 `2F2549538541BCB32AA7529F806E842A2B35515B795D9312C39E037DF36BD92D`。
- 部署驗收：`docker compose -p stock-forecasting-ticket-09-stage6-acceptance -f compose.yaml --profile ticket-09-acceptance run --build --rm ticket-09-acceptance` 輸出 `status=passed`，八個 deployed fail-closed checks 全為 `true`；專用 containers／network／volumes 與 PostgreSQL integration-test resources 已在驗收後移除。
