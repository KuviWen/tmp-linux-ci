...
- 每次 shadow 前重新解析該候選最新的 append-only GateDecision；核准後新增的 hard-gate veto 會以 `hard_gate_vetoed` 保存 blocked shadow evidence，不能沿用舊核准增加 eligible cycle。Model／governance artifacts 與 state-store content digests 共用 canonical UTF-8 bytes 與 SHA-256 helpers，避免 Unicode policy／owner 值產生模組相依 ID。
...
- 驗證：受影響 contract tests `115 passed`；完整非 PostgreSQL suite `455 passed, 1 deselected`，PostgreSQL opt-in `1 passed, 455 deselected`；`mypy src tests`（106 source files）、`ruff check src tests`、`ruff format --check src tests`（106 files）、wheel build、Compose config 與 Ticket 09 Compose acceptance 均通過。Wheel SHA-256 為 `F94BCF259879E83BCBF293754188C8EB6259033282F9E3B76867363204B4251C`。
- 部署驗收：`docker compose -p stock-forecasting-ticket-09-cost-review -f compose.yaml --profile ticket-09-acceptance run --build --rm ticket-09-acceptance` 輸出 `status=passed`，九個 deployed fail-closed checks 全為 `true`，包括使用實際 `stock` role 的 `lifecycle_ledger_append_only=true`；專用 containers／network／volumes 與 PostgreSQL integration-test resources 已在驗收後移除。持久化 `stock-forecasting-ticket-09-operator` 以 qualified policy 保留原 volumes 重建後，已 cold-read 正式成本 manifest `sha256:6f70e94386c154dc4b5029d23a6d8a8ca3c6e7a7bcabe92199e66b7a1bd68cc7`。
