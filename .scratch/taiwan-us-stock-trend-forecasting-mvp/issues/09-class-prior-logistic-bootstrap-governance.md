...
- [ ] 每個市場按季度使用全部合格歷史建立明確 training／validation spans、固定 20-session purge、固定 20-session embargo 及一季一次性 test；fold manifest 記錄實際深度與 fold 數，統計／類別／校準支援不足時不得形成正式候選。
