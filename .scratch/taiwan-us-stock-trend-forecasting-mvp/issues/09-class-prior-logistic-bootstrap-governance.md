...
- 每次 shadow 前重新解析該候選最新的 append-only GateDecision；核准後新增的 hard-gate veto 會以 `hard_gate_vetoed` 保存 blocked shadow evidence，不能沿用舊核准增加 eligible cycle。治理 artifacts 共用 canonical UTF-8 bytes 與 raw SHA-256 identity helper，避免 Unicode policy／owner 值產生模組相依 ID。
...
- 驗證：完整非 PostgreSQL suite `447 passed, 1 deselected`，PostgreSQL opt-in `1 passed, 447 deselected`；`mypy src tests`（103 source files）、`ruff check src tests`、`ruff format --check src tests`（103 files）、wheel build、Compose config 與 Ticket 09 Compose acceptance 均通過。Wheel SHA-256 為 `7C1E5788F89DB0530C8D7CDE53F8FC3A670241FF1A6758C2D698EBE1D5767046`。
- 部署驗收：`docker compose -p stock-forecasting-ticket-09-review5 -f compose.yaml --profile ticket-09-acceptance run --build --rm ticket-09-acceptance` 輸出 `status=passed`，九個 deployed fail-closed checks 全為 `true`，包括使用實際 `stock` role 的 `lifecycle_ledger_append_only=true`；專用 containers／network／volumes 與 PostgreSQL integration-test resources 已在驗收後移除。
