...
- Bootstrap 改善幅度由 candidate 中保存的 class-prior／logistic equal-cell macro-F1 原始分數重新計算，並要求與 submitted percentage points 一致；不能只靠 caller 自報改善通過。Model artifacts 的 calibrator bundle 欄位必填；未提供 validation rows 時，每個模型 cell 會寫入內容定址的 `insufficient_data` identity evidence，缺失、空缺 formal cells 或竄改仍 fail closed。
...
- Shadow cycle 必須通過 `ShadowEligibilityVerifier` 的雙市場共同合格 EOD 驗證；預設 verifier 不可用，且 provider 例外也以 `shadow_eod_not_eligible` fail closed。Contract evidence 使用真實遞增工作日，不再把週末自我宣告成合格交易日。
...
- 驗證：完整非 PostgreSQL suite `438 passed, 1 deselected`，PostgreSQL opt-in `1 passed, 438 deselected`；`mypy src tests`（102 source files）、`ruff check .`、`ruff format --check .`、wheel build、Compose config 與 Ticket 09 Compose acceptance 均通過。Wheel SHA-256 為 `48C9C4E2D1AF7C818731815BB0F02B140F53CBB5C19EEDE44B9F24F64B6DCAC9`。
- 部署驗收：`docker compose -p stock-forecasting-ticket-09-review -f compose.yaml --profile ticket-09-acceptance run --build --rm ticket-09-acceptance` 輸出 `status=passed`，八個 deployed fail-closed checks 全為 `true`；專用 containers／network／volumes 與 PostgreSQL integration-test resources 已在驗收後移除。
