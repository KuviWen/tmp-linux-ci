...
**What to build:** 從 P3 passing bundle 重現 shared neural 候選的增量建構、三 seeds、八季回測、baselines／ablations、HPO、六 calibrators、Integrated Gradients、無網路重現、hard gates、人工核准、五次 shadow、promotion／rollback／stale／drift／policy 情境，並發布不可變 P4 acceptance bundle；neural 未勝出時允許 logistic 合法留任。
...
- [ ] Acceptance run 從 P3 bundle、100＋100 manifests、processing bundles、baseline assignment 與 approved runtime 重建 price-only、modality increments 及完整 shared candidate。
- [ ] 三 seeds、最新八季 folds、class-prior／logistic／price-only／逐模態 ablations、六 calibrators、cost／support slices、HPO lineage 及 immutable reports 全部可重現。
...
- [ ] 研究介面在 100＋100 展示 neural／baseline assignment、三期間預測、信心、支援、主要影響、gate reliance、文件證據、history、backtests、gate／reproduction lineage 及繁中無障礙。
