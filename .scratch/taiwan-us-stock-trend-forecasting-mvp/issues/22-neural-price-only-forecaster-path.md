...
- [ ] Neural price-only 與 logistic 在 verified-history manifest 實際可形成的相同 eligible folds、market／horizon cells、calibration、cost 及 support slices 上比較；樣本不足的 cell 明確 unavailable，結果進 immutable EvaluationReport 與 research governance view。
- [ ] Shadow EOD 對完整 measured support pool 產生與 logistic 相同 shape 的 result-or-reason、REST／UI、health、latency 與 audit evidence，請求順序及批次組成不改變結果。
