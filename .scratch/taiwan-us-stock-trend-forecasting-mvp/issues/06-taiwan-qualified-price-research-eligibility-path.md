...
- Durable source checkpoints advance only with the atomic evidence publication. Raw content identity is separate from append-only retrieval receipts, and historical publication additionally requires a platform-observed `HistoricalAvailabilityClaim` with session, schema, integrity, company-action, and lifecycle checks.
- Formal research qualification additionally requires the versioned manifest's exact current／historical source lineage, historical claim, and an existing immutable qualification artifact. Synthetic provider-contract rows therefore remain non-formal.
...
- Verified: `python -m pytest -m "not postgresql" -q` (156 passed), ticket／authorization-focused pytest (51 passed), `python -m mypy src tests`, `python -m ruff check .`, `python -m ruff format --check .`, and an isolated wheel build.
