...
- Verified: `python -m pytest -m "not postgresql" -q` (160 passed, 1 deselected), ticket／contract-focused pytest (26 passed), `python -m mypy src tests`, `python -m ruff check src tests`, `python -m ruff format --check src tests`, and an isolated `pip wheel --no-deps --no-build-isolation` build.
