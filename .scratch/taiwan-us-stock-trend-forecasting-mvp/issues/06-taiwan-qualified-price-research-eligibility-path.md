...
- Every listing and operations eligibility read resolves the currently effective policy／entitlement and six required uses for every non-blocked source. Immutable materialization authorization supplies only the prior subject and the exact dataset／evaluation／decision／trace／correlation binding; it is never rehydrated as a current authenticated `SecurityContext`. The request principal is authorized only for the eligibility-metadata read. Every current-rights result, including missing／invalid evidence and unavailable policy resolution, is a separate content-addressed `current_source_rights_resolution` artifact linked to the query trace. Revoke, suspend, replacement, or use removal overlays `policy_blocked` on published／quarantined projections before the historical decision's `valid_until`, without rewriting that evidence. A rate-limited source row remains historically `deferred`, while the listing aggregate fails closed as `policy_blocked`; UI reports both the later rights denial and the earlier checkpoint-preserving rate limit.
...
- Verified: `python -m pytest -m "not postgresql" -q` (164 passed, 1 deselected), ticket／authorization-focused pytest (49 passed), `python -m mypy src tests`, `python -m ruff check src tests`, `python -m ruff format --check src tests`, and an isolated `pip wheel --no-deps --no-build-isolation` build (`sha256:7c9449a0b07f0ba8fdeff6a37c6419368bf6196496b4ea67d698c71720681d90`). The unfiltered suite reached 164 passing tests and only the PostgreSQL environment test failed on connection timeout.
