...
- Every listing eligibility read re-evaluates the currently resolved source policy／entitlement and six required uses; revoke, suspend, replacement, or use removal blocks REST／UI even before the materialization decision's `valid_until`. Provider rate limiting is an independent `deferred` state: policy remains passed, no raw／dataset is claimed, and UI reports that checkpoint did not advance.
