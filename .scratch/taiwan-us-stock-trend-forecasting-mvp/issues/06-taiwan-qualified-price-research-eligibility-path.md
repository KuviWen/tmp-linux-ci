...
- Every listing and operations eligibility read re-evaluates the currently resolved policy／entitlement and six required uses for every non-blocked source using the workload principal recovered from immutable materialization authorization; the request principal is used only for eligibility-metadata read authorization. Revoke, suspend, replacement, or use removal overlays `policy_blocked` on published／quarantined projections before the historical decision's `valid_until`, without rewriting that evidence. Each projection exposes a separately traceable `current_policy_decision`. Provider rate limiting remains `deferred`, but its current policy check and decision still show a later denial; no raw／dataset is claimed and UI reports that checkpoint did not advance.
