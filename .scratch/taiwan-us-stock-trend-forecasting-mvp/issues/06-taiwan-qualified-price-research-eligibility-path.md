...
- Verified: `python -m pytest -m "not postgresql" -q` (166 passed, 1 deselected), ticket／authorization-focused pytest (51 passed), `python -m mypy src tests`, `python -m ruff check src tests`, `python -m ruff format --check src tests`, and an isolated `pip wheel --no-deps --no-build-isolation` build (`sha256:e8a6acedd27898e5bf21d5ed9027d1dfde9d44d1d4830f320b3f29dcd6c5c424`). The unfiltered suite reached 166 passing tests and only the PostgreSQL environment test failed on connection timeout.
