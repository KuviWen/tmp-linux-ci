...
- `source_retrieval_receipt` 由 data-supply 與 historical-evidence 路徑共用同一 typed payload／schema owner。相同 source、distribution 與 content object 的再次觀測仍追加本次 immutable receipt，但新 attestation 解析並保存最早 receipt 的 ID 與 `first_observed_at`，另以 `attested_at` 表示本次平台觀測時間。
