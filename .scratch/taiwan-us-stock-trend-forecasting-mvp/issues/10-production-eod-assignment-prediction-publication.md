...
- Production publication authorizes the catalog before replay／selection access, re-evaluates the selected manifest after resolution, and reloads current scoped action／source-use rights through the active publication transaction at the persistence boundary; denied commit-time decisions roll back prediction, research, outbox, and operations writes. Replay, research display, operations reads, and notifications also re-evaluate their scoped rights.
...
- Verified with focused acceptance／contract tests, all acceptance＋contract tests (535 passed), the full repo suite against a clean disposable PostgreSQL 17 container (536 passed), the PostgreSQL-only migration／role／schema／rollback integration (1 passed), strict mypy (109 files), Ruff lint／format (109 Python files), Alembic `20260819_09 (head)`, and wheel SHA-256 `2a57b6cf23d88ec3e5d73c2ad6ed0aae9ea31a78d32022873154eec986d183de`.
- Windows Docker Desktop 4.84.0／Engine 29.6.2／Compose 5.3.1 validated both Compose models. A clean `compose.test.yaml` run built the application image, initialized SCRAM PostgreSQL, applied every migration through `20260819_09`, and connected as non-superuser `stock_test`; an isolated deployed acceptance run returned `status=passed` with all nine checks true, while an isolated operator-profile smoke proved PostgreSQL／API／ingress／CLI health and persisted the three production scopes in the owner policy. All disposable projects and volumes were removed afterward.
- The general P1 Windows platform run itself returned `platform_run_status=passed`, but its aggregate bundle remained blocked only at `GATE-DEPLOY-01` because no `linux_ci` counterpart for this exact commit exists. That unrelated cross-platform release gate is not claimed as passing; `formal_model_qualification=not_claimed` also remains unchanged.
